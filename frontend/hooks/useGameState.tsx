...
import { usePowerSyncGameState, type PSGameCharacter, type PSGameMessage } from "@/hooks/usePowerSyncGameState";
...
export function GameStateProvider({ children, onCharacterResponse }: GameStateProviderProps) {
  ...
  // Track the active session_id for PowerSync subscriptions
  const [psSessionId, setPsSessionId] = useState<string | null>(null);
  const ps = usePowerSyncGameState(psSessionId);

  // When PowerSync characters update, merge into session state
  // This keeps the existing session object in sync without breaking SSE flow
  useEffect(() => {
    if (!ps.characters.length || !session) return;
    const psChars: CharacterPublic[] = ps.characters.map((c: PSGameCharacter) => ({
      id: c.id,
      name: c.name,
      persona: c.persona || "",
      speaking_style: c.speaking_style || "",
      avatar_seed: c.avatar || "",
      public_role: c.public_role || "",
      voice_id: "",
      is_eliminated: !!c.is_eliminated,
    }));
    // Only update if character elimination status actually changed
    const eliminationChanged = session.characters.some((sc) => {
      const pc = psChars.find((p) => p.id === sc.id || p.name === sc.name);
      return pc && pc.is_eliminated !== sc.is_eliminated;
    });
      ...
      setSession((prev) => {
        ...
        return {
          ...
          characters: prev.characters.map((sc) => {
            const pc = psChars.find((p) => p.id === sc.id || p.name === sc.name);
            if (pc && pc.is_eliminated !== sc.is_eliminated) {
              return { ...sc, is_eliminated: pc.is_eliminated };
            }
            return sc;
          }),
        };
  ...
  }, [ps.characters, session]);

  // Sync PowerSync game session phase/round (authoritative from DB)
  useEffect(() => {
    if (!ps.gameSession) return;
    if (ps.gameSession.winner) {
      setGameEnd((prev) => prev ? prev : { winner: ps.gameSession!.winner! });
    }
    if (ps.gameSession.round) {
      setRound((prev) => ps.gameSession!.round !== prev ? ps.gameSession!.round : prev);
    }
    if (ps.gameSession.tension_level !== undefined) {
      setTension((prev) => ps.gameSession!.tension_level !== prev ? ps.gameSession!.tension_level : prev);
    }
  }, [ps.gameSession]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Start PowerSync subscriptions for this session
        setPsSessionId(data.session_id);
  ...
  const handleSessionCreated = useCallback((sess: GameSession) => {
    ...
    // Start PowerSync subscriptions for this session
    setPsSessionId(sess.session_id);
  }, []);
