"""GameOrchestrator — manages game sessions with in-memory state + Supabase/Redis persistence."""
...
class GameOrchestrator:
    ...
    def _stamp_message(self, msg: ChatMessage, emotion: str = "") -> ChatMessage:
    ...
    async def _persist_message(self, session_id: str, msg: ChatMessage):
    ...
    async def _persist_vote(self, session_id: str, vote, round_num: int):
    ...
    async def _persist_night_action(self, session_id: str, action, round_num: int):
    ...
    async def handle_open_discussion(self, session_id: str) -> AsyncGenerator[str, None]:
        ...
        for i, char in enumerate(speakers):
            ...
            try:
                ...
                self._stamp_message(ai_msg, emotion=dominant_emotion)
                state.messages.append(ai_msg)
                await self._persist_message(session_id, ai_msg)
    ...
    async def handle_chat(
        ...
        self._stamp_message(player_msg)
        state.messages.append(player_msg)
        await self._persist_message(session_id, player_msg)
            ...
            if non_responders and reactor_agent:
                try:
                    ...
                    if reaction:
                        ...
                        self._stamp_message(react_msg, emotion=reactor_emotion)
                        state.messages.append(react_msg)
                        await self._persist_message(session_id, react_msg)
    ...
    async def handle_vote(
        ...
        for vote in vote_result.votes:
            await self._persist_vote(session_id, vote, state.round)
        ...
        if player_action_type:
            ...
            if player_action_type == "kill" and state.player_role:
                ...
                for aid in state.player_role.allies:
                    ...
                    self._stamp_message(night_msg, emotion=dominant_emotion)
                    state.messages.append(night_msg)
                    await self._persist_message(session_id, night_msg)
    ...
    async def handle_night_chat(
        ...
        self._stamp_message(player_whisper)
        state.messages.append(player_whisper)
        await self._persist_message(session_id, player_whisper)
        ...
        for ally in allies:
            ...
            self._stamp_message(ally_msg, emotion=dominant_emotion)
            state.messages.append(ally_msg)
            await self._persist_message(session_id, ally_msg)
    ...
    async def _resolve_night(
        ...
        for action in state.night_actions:
            ...
            await self._persist_night_action(session_id, action, state.round)
        ...
        eliminated_ids = list(dict.fromkeys(a.target_id for a in state.night_actions if a.result == "killed" and a.target_id))
